...
while ($row = $result->fetch_assoc()) {
    switch ($row['jenis_studio']) {
        case 'Regular':
            $tiketRegular[] = new TiketRegular($row);
            break;
        case 'IMAX':
            $tiketIMAX[] = new TiketIMAX($row);
            break;
        case 'Velvet':
            $tiketVelvet[] = new TiketVelvet($row);
            break;
    }
}
        ...
        <div class="studio-section regular">
            ...
            <div class="tiket-grid">
                <?php if (count($tiketRegular) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p style="color: #999;">Belum ada data tiket Regular.</p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="studio-section imax">
            ...
            <div class="tiket-grid">
                <?php if (count($tiketIMAX) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p style="color: #999;">Belum ada data tiket IMAX.</p>
                <?php endif; ?>
            </div>
        </div>
                ...
                <?php if (count($tiketVelvet) > 0): ?>
                    <?php foreach ($tiketVelvet as $tiket): ?>
                        <div class="tiket-card premium">
                            ...
                            <div class="total-harga premium-price">
                                <strong>Total Harga:</strong> Rp <?= number_format($tiket->hitungTotalHarga(), 0, ',', '.') ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p style="color: #999;">Belum ada data tiket Velvet.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
